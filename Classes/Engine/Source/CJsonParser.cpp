...
bool CJsonParser::ParseCallback(RefJsonNode a_rListener, CEntityNode* a_pEntity)
{
	...
	{
		CEventCallback oCallback("CountdownPressed",m_pKernel, &CKernel::CountdownPressed,
			SEvent(SEvent::NONE,a_pEntity));
		a_pEntity->AddListener(sType, oCallback);
	}
	else if (sCallbackString == "CountdownReleased")
	{
		CEventCallback oCallback("CountdownReleased",m_pKernel, &CKernel::CountdownReleased,
			SEvent(SEvent::NONE,a_pEntity));
		a_pEntity->AddListener(sType, oCallback);
	}
	else if (sCallbackString == "ValidateTeamTask")
	{
		CEventCallback oCallback("ValidateTeamTask",m_pKernel, &CKernel::ValidateTeamTask,
			SEvent(SEvent::NONE,a_pEntity));
		a_pEntity->AddListener(sType, oCallback);
	}
	else
